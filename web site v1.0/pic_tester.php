...
<body>
    <br>
    <div align="center">     <center> <img align="middle" alt="head" src="files/head_pic.png" /></center> 
    <br>
    <br>
    <br>
<?php
...
if (move_uploaded_file($_FILES['pic']['tmp_name'],$target_path) && getimagesize($target_path))
	...
	include("watermark_image.class.php");
	  ...
	  // Watermark image
	   ...
	   $iscale=$qrsize/(max($isize[0],$isize[1]));
	   $shrink_percentage=1;
     ...
     if($publish){
      $file=fopen("latest","r");
      $temp=fgets($file).fgets($file).fgets($file).fgets($file);
      fclose($file);
	  $file=fopen("latest","w");
      fputs($file,"http://".$url."\n".$imgdestpath."\n");
      fputs($file,$temp);
      fclose($file);
      ?>  <script type="text/javascript">
    parent.frames[0].location =" last.html" ;
    </script>
            
            <?
     }
	  ?><div align="center"><img src="<? echo $imgdestpath; ?>"  alt="<? echo $imgdestpath; ?> (USER_picture)">
